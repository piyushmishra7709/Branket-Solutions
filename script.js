...
// ---- WHERE ENQUIRIES GET SAVED ----
// This site has no backend of its own, so form submissions need somewhere
// real to land. The form below is wired to POST to Formspree
// (https://formspree.io) — a free service that receives the submission,
// emails it to you, and keeps a copy in a dashboard you can log into.
//
// To make it live:
...
// Until that's done, the form still works — it falls back to opening
// email/WhatsApp with the details pre-filled, so no enquiry is lost either way.
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Work page — "View more" reveals additional concept projects
  const viewMoreBtn = document.querySelector("#view-more-projects");
  if (viewMoreBtn){
    const grid = document.querySelector("#projects-grid");
    viewMoreBtn.addEventListener("click", () => {
      grid.classList.add("show-extras");
      grid.querySelectorAll(".project-extra").forEach(card => card.classList.add("show"));
      viewMoreBtn.style.display = "none";
      // Re-apply the currently active filter so newly revealed cards respect it
      const activeBtn = document.querySelector(".filter-btn.active");
      if (activeBtn) activeBtn.click();
      grid.querySelectorAll(".project-extra.show").forEach(el => {
  ...
  if (filterBtns.length && projectCards.length){
    ...
    const applyFilter = (key) => {
      projectCards.forEach(card => {
        ...
        const matches = key === "all" || cats.includes(key);
        const isExtra = card.classList.contains("project-extra");
        const extrasVisible = grid ? grid.classList.contains("show-extras") : true;
        card.classList.toggle("show", matches && (!isExtra || extrasVisible));
      });
    };
    applyFilter("all");
  ...
  if (form){
    ...
    const fallbackToEmailAndWhatsapp = (summary, name) => {
      const subject = encodeURIComponent(`New inquiry — ${name || "Website visitor"}`);
      const body = encodeURIComponent(summary);
      const mailtoUrl = `mailto:${EMAIL_ADDRESS}?subject=${subject}&body=${body}`;
      status.innerHTML = `Thanks — your details are ready to send. If your email app didn't open, ` +
        `<a href="${mailtoUrl}" style="color:var(--orange); text-decoration:underline;">click here to email us</a>, ` +
        `or <a href="${waLink('Hi Branket Solutions, here are my project details:\n' + summary)}" target="_blank" rel="noopener" style="color:var(--orange); text-decoration:underline;">send it on WhatsApp instead</a>.`;
    ...
    form.addEventListener("submit", async (e) => {
      ...
      if (!isFormspreeConfigured()){
        // No live endpoint set up yet — route straight to email/WhatsApp so
        // nothing gets lost, and flag it so the site owner knows to finish setup.
        fallbackToEmailAndWhatsapp(summary, name);
        return;
      }
